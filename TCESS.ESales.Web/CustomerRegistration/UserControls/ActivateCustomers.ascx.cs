...
public partial class CustomerRegistration_UserControls_ActivateCustomers : BaseUserControl
{
    ...
    private void GetCustomersForActivation()
    {
        ...
        lstCustomer = ESalesUnityContainer.Container.Resolve<ICustomerService>().GetCustomersByCustomerStatus(true);
    ...
    protected void grdActivateCustomers_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            GridViewRow row = (GridViewRow)grdActivateCustomers.Rows[e.RowIndex];
            TextBox txtCustomerCode = (TextBox)row.FindControl("txtCustomerCode");

            CustomerDTO customerDetials = ESalesUnityContainer.Container.Resolve<ICustomerService>().GetCustomerDetailsByCode(txtCustomerCode.Text.Trim());

            if (customerDetials.Cust_Id == 0)
            {
                ExceptionHandler.AppExceptionManager.Process(() =>
                    {
                        ...
                        customerDetails.Cust_Code = ((TextBox)grdActivateCustomers.Rows[e.RowIndex].FindControl("txtCustomerCode")).Text;
                        ...
                        ucMessageBoxForGrid.ShowMessage(Messages.CustomerIsActivated);
                    }, Globals.ExceptionTypes.ExceptionShielding.ToString());
            }
            else
            {
                ucMessageBox.ShowMessage("Customer code is already alloted to other customer");
            }
        }
        catch (Exception ex)
        {
