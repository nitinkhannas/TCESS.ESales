...
public partial class Reports_UserControls_DailyBookingStatusDataWithBarChart : BaseUserControl
{
    ...
    private void GenerateDailyBookingStatusDataReport(DateTime fromDate, DateTime toDate)
    {
        if (fromDate != null)
        {
            IList<object> lstBookingStatusData = ESalesUnityContainer.Container.Resolve<IReportService>()
                .GetDailyBookingStatusReportForBarChart(base.GetAgentByUserId().UAM_Agent_Id, Convert.ToDateTime(fromDate),
            ...
            if (lstBookingStatusData.Count > 0)
            {
                var bookingStatusData = new
                {
                    ...
                    truckOuts = lstBookingStatusData[3],
                    prevDayBookings = lstBookingStatusData[4],
                    pendings = lstBookingStatusData[5]
                };
                ...
                if (bookingStatusDataList.Count > 0)
                {
