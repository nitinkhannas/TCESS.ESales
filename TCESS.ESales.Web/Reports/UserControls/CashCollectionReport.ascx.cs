...
public partial class Reports_UserControls_CashCollectionReport : BaseUserControl
{
    ...
    public void LoadReport(int agentId, DateTime fromDate, DateTime toDate)
    {
        ...
        Common.SetReportEmbeddedResource(reportViewer, "TCESS.ESales.CommonLayer.Reports.CashCollectionReport.rdlc");

        //Set datasource for cash collection report
        IList<MoneyReceiptDTO> lstMoneyReceiptDTO = ESalesUnityContainer.Container.Resolve<IReportService>()
            .GetCashCollectionReport(agentId, fromDate, toDate);
        ReportDataSource CashCollectionDataSource = new ReportDataSource("dsCashCollection", lstMoneyReceiptDTO);
