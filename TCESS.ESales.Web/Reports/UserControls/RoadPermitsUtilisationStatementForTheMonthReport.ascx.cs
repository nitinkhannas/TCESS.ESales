...
public partial class Reports_UserControls_RoadPermitsUtilisationStatementForTheMonthReport : BaseUserControl 
{
    ...
    public void LoadReport(int month, int year)
    {
        ...
        Common.SetReportEmbeddedResource(reportViewer, "TCESS.ESales.CommonLayer.Reports.RoadPermitsUtilisationStatementForTheMonth.rdlc");

        //Set datasource for loading advice
        IList<SettlementOfAccountsDTO> lstLoadingAdviceRpt = ESalesUnityContainer.Container.Resolve<IReportService>()
            .GetDFormutilizationStatementForTheMonthData(base.GetAgentByUserId().UAM_Agent_Id, month, year);
        ReportDataSource loadingAdviceDataSource = new ReportDataSource("RoadPermitsUtilisationStatementForTheMonth", lstLoadingAdviceRpt);
