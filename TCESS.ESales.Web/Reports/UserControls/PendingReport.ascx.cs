...
public partial class Reports_UserControls_PendingReport : BaseUserControl
{
    ...
    public void LoadReport(int agentId,DateTime fromDate, DateTime toDate)
    {
        ...
        Common.SetReportEmbeddedResource(reportViewer, "TCESS.ESales.CommonLayer.Reports.BookingPendingReport.rdlc");
        
        //Set datasource for pending bookings
        IList<BookingDTO> lstPendingRpt = ESalesUnityContainer.Container.Resolve<IReportService>()
            .GetPendingReport(agentId, fromDate, toDate);
        ReportDataSource loadingAdviceDataSource = new ReportDataSource("dsBookingPending", lstPendingRpt);
