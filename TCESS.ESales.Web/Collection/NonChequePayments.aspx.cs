...
public partial class GhatoCollection_NonChequePayments : BasePage
{
    ...
    {
        IList<ConsolidatedReportDTO> lstConsolidatedReport = ESalesUnityContainer.Container.Resolve<IPaymentService>().GetReportForNonChequePayments();
    ...
    private void LoadPaymentTransitDetails()
    {
        IList<object> lstPaymentTransit = ESalesUnityContainer.Container.Resolve<IPaymentService>()
            .GetCollectionHeaderForReports((int)HelperClass.PaymentModes.CASH, (int)Globals.PaymentHeader.FORREPORTSCREEN);
